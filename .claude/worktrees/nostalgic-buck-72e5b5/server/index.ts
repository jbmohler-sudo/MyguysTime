...
app.post("/api/auth/register", asyncHandler(async (req, res) => {
  const { fullName, email, password, companyName } = req.body as {
    fullName?: string;
    email?: string;
    password?: string;
    companyName?: string;
  ...
  let stateRule = await prisma.statePayrollRule.findUnique({ where: { stateCode: "MA" } });
  if (!stateRule) {
    stateRule = await prisma.statePayrollRule.findFirst({ where: { isActive: true } });
  }
  if (!stateRule) {
    res.status(500).json({ error: "Unable to create account: payroll rules not configured." });
    return;
  }
  ...
  const company = await prisma.company.create({
    data: { companyName: companyName.trim(), stateCode: stateRule.stateCode },
  });
  ...
  await prisma.companyPayrollSettings.create({
    data: {
      ...
      defaultStateWithholdingMode: stateRule.defaultStateWithholdingMode,
      defaultStateWithholdingValue: stateRule.defaultStateWithholdingValue,
      timeTrackingStyle: "FOREMAN",
      defaultLunchMinutes: 30,
      payType: "HOURLY_OVERTIME",
      trackExpenses: true,
      pfmlEnabled: stateRule.defaultPfmlEnabled,
      pfmlEmployeeRate: stateRule.defaultPfmlEmployeeRate,
      supportLevelSnapshot: stateRule.supportLevel,
    },
  });
  ...
  const token = issueToken({ userId: user.id, role: "ADMIN", companyId: company.id });
